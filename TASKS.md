...
- Reworked the workspace sidebar header in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue) so the home entry and mobile close button stay in the top bar while the workspace switcher moves under the current `文档/分享` section heading
- Updated sidebar layout styling in [main.css](/mnt/e/workspace/flareDocs/app/assets/css/main.css) to keep the new hierarchy readable on both mobile and desktop, including dark-theme text balance
- Logged the sidebar layout change in [README.md](/mnt/e/workspace/flareDocs/README.md)
- Refined the same sidebar again in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue): the home entry is now a labeled `主页` button, and the four tree actions are folded into a single overflow menu so the workspace switcher can use the full row width
- Added matching sidebar button/menu styling in [main.css](/mnt/e/workspace/flareDocs/app/assets/css/main.css) for both light and dark themes
- Updated [app.vue](/mnt/e/workspace/flareDocs/app/app.vue) so localhost development actively unregisters the service worker and clears flareDocs caches, preventing stale `/_nuxt` assets from breaking page interactivity after UI changes
- Tightened the same sidebar controls again in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue) and [main.css](/mnt/e/workspace/flareDocs/app/assets/css/main.css) so the workspace switcher and overflow menu share one aligned row instead of appearing vertically offset
- Reordered the workspace overflow menu in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue) so `新增文档 / 新增目录` come first, and added dedicated locale copy in [useAppLocale.ts](/mnt/e/workspace/flareDocs/app/composables/useAppLocale.ts)
- Simplified the sidebar home entry in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue) and [main.css](/mnt/e/workspace/flareDocs/app/assets/css/main.css) from icon+label to icon-only, keeping the tooltip/ARIA label while reclaiming horizontal space
- Adjusted the team-space access model in [spaces.ts](/mnt/e/workspace/flareDocs/app/server/utils/spaces.ts), [index.get.ts](/mnt/e/workspace/flareDocs/app/server/api/spaces/index.get.ts), and [useAppLocale.ts](/mnt/e/workspace/flareDocs/app/composables/useAppLocale.ts): all signed-in users now see `team` spaces by default as `editor`, so documents can be viewed and collaboratively edited without extra grants, while space-level admin actions still require explicit admin membership
- Expanded shared-document actions in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue), [WorkspaceIcon.vue](/mnt/e/workspace/flareDocs/app/components/WorkspaceIcon.vue), and [useAppLocale.ts](/mnt/e/workspace/flareDocs/app/composables/useAppLocale.ts): all document menus now include `复制内容`, `分享给我` gains export/copy/info/fullscreen, and `我分享的` keeps the share list plus an entry back to the original editable workspace document
- Fixed shared-document info behavior in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue) and [useAppLocale.ts](/mnt/e/workspace/flareDocs/app/composables/useAppLocale.ts): the info modal now renders for shared views too, includes the workspace name, and the `我分享的 -> 编辑` jump uses an explicit route object back to the source workspace document
...
- Reduced workspace first-open overhead in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue), [tree.get.ts](/mnt/e/workspace/flareDocs/app/server/api/spaces/[spaceId]/tree.get.ts), and [api.ts](/mnt/e/workspace/flareDocs/app/types/api.ts): the space switcher list is now fetched only when opened, first-time tree load no longer auto-expands every folder, and sibling nodes are ordered by newest-created first to surface recent docs without expanding the full tree.
- Converted the workspace tree in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue) and [tree.get.ts](/mnt/e/workspace/flareDocs/app/server/api/spaces/[spaceId]/tree.get.ts) to lazy branch loading: initial entry now requests only root nodes, expanding a folder fetches just that branch, and explicit full-tree actions (`全部展开` / move target picker) trigger a full fetch with progress feedback.
- Hardened auth session persistence in [auth.ts](/mnt/e/workspace/flareDocs/app/server/utils/auth.ts), [login.post.ts](/mnt/e/workspace/flareDocs/app/server/api/auth/login.post.ts), and [me.get.ts](/mnt/e/workspace/flareDocs/app/server/api/auth/me.get.ts): secure-cookie detection now respects forwarded HTTPS headers, session cookies use `SameSite=Strict`, and redundant login-time bootstrap/personal-space work was removed from hot paths to reduce refresh relogin risk and post-login latency.
- Added document-level direct sharing for personal workspaces in [db/schema.ts](/mnt/e/workspace/flareDocs/db/schema.ts), [db/migrations/0004_document_shares.sql](/mnt/e/workspace/flareDocs/db/migrations/0004_document_shares.sql), [app/server/api/shares](/mnt/e/workspace/flareDocs/app/server/api/shares), and [app/server/api/spaces/[spaceId]/docs/[docId]/shares\*](/mnt/e/workspace/flareDocs/app/server/api/spaces/[spaceId]/docs/[docId]/shares.get.ts): personal docs can now be shared read-only with specific users without creating extra team workspaces.
- Updated homepage, shared reading pages, and workspace sharing UI in [index.vue](/mnt/e/workspace/flareDocs/app/pages/index.vue), [shared/index.vue](/mnt/e/workspace/flareDocs/app/pages/shared/index.vue), [shared/[documentId].vue](/mnt/e/workspace/flareDocs/app/pages/shared/[documentId].vue), and [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue): recipients now get a dedicated “Shared With Me” entry point on mobile/desktop, and personal documents expose a share management modal instead of relying on team spaces for ad-hoc sharing.
- Fixed share API runtime compatibility in [app/server/api/shares/index.get.ts](/mnt/e/workspace/flareDocs/app/server/api/shares/index.get.ts) and [app/server/api/shares/[documentId].get.ts](/mnt/e/workspace/flareDocs/app/server/api/shares/[documentId].get.ts): removed `drizzle-orm` `alias` imports that were causing browser requests to fail with a 500 in the current dependency/runtime combination.
- Hardened local sharing diagnostics in [app/server/utils/shares.ts](/mnt/e/workspace/flareDocs/app/server/utils/shares.ts) and share APIs: if the local D1 database has not applied [0004_document_shares.sql](/mnt/e/workspace/flareDocs/db/migrations/0004_document_shares.sql), the UI now receives a clear migration-required error instead of a raw `no such table: document_shares` stack. The sharing action remains in the workspace “more tools” area instead of adding a second button below the document title.
- Fixed two local compatibility regressions: [app/server/utils/spaces.ts](/mnt/e/workspace/flareDocs/app/server/utils/spaces.ts) now treats legacy-named personal spaces as personal workspaces for share-entry eligibility, and [MarkdownEditor.client.vue](/mnt/e/workspace/flareDocs/app/components/MarkdownEditor.client.vue) now statically imports ByteMD to avoid the dev-time dynamic-import failure that left the editor area blank.
...
- Tuned dark-theme homepage onboarding aesthetics in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue) + [main.css](/mnt/f/newworkspace/flareDocs/app/assets/css/main.css): introduced dedicated guide card/item classes and dark-mode overrides for panel/background/text contrast so the signed-in quick guide no longer appears washed-out in dark skin.
- Adjusted homepage signed-in guidance card in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): restored quick-start onboarding copy while removing private/team/public metric pills from that panel for a cleaner, instruction-only presentation.
- Refined the homepage signed-in guidance card in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): replaced step-by-step onboarding with a concise "space type selection" guide (private/team/public usage scope + default recommendation), with localized copy in [useAppLocale.ts](/mnt/f/newworkspace/flareDocs/app/composables/useAppLocale.ts).
- Updated homepage right-panel onboarding in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue) + [useAppLocale.ts](/mnt/f/newworkspace/flareDocs/app/composables/useAppLocale.ts): removed the top-right "open workspace" shortcut from the session panel and replaced it with a compact 4-step quick-start guide (localized zh/en) for clearer first-time usage.
- Refined homepage workspace entry layout in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): fixed create-workspace icon placement (after user-management), removed redundant right-panel copy blocks, removed "space total" metric card, and kept only a single primary "open workspace" action.
...
- Updated homepage profile settings UX in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): desktop profile settings changed from inline expanding panel to modal overlay (mobile drawer kept), eliminating layout jump/page stretch when toggling settings; added matching dark-mode modal styles in [main.css](/mnt/f/newworkspace/flareDocs/app/assets/css/main.css).
- Added client-only voice-to-Markdown append in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue): edit mode now supports browser speech recognition (start/stop/preview/append), and recognized text is auto-appended under today's `## YYYY-MM-DD` section as list items; added icon support in [WorkspaceIcon.vue](/mnt/f/newworkspace/flareDocs/app/components/WorkspaceIcon.vue), localized labels in [useAppLocale.ts](/mnt/f/newworkspace/flareDocs/app/composables/useAppLocale.ts), and voice panel theme styles in [main.css](/mnt/f/newworkspace/flareDocs/app/assets/css/main.css).
- Refined voice-entry UX in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue): moved voice trigger into the workspace "more/actions" menu and switched to a closable floating panel above the editor with manual text editing before append, so editing layout stays clean.
- Fixed production microphone permission regression in [00-security-headers.ts](/mnt/f/newworkspace/flareDocs/app/server/middleware/00-security-headers.ts): `Permissions-Policy` now allows same-origin microphone access (`microphone=(self)`) instead of globally disabling it (`microphone=()`), so browser "Allow microphone" can take effect for voice append.
- Finalized voice panel interaction in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue) + [main.css](/mnt/f/newworkspace/flareDocs/app/assets/css/main.css): voice entry remains under actions menu, active listening icon now pulses, manual-text panel removed the redundant clear `X` action button, and date insertion logic now keeps today's section pinned to the top of the Markdown document when appending.
- Added first-pass PWA install support: manifest + service worker + runtime registration ([manifest.webmanifest](/mnt/f/newworkspace/flareDocs/public/manifest.webmanifest), [sw.js](/mnt/f/newworkspace/flareDocs/public/sw.js), [app.vue](/mnt/f/newworkspace/flareDocs/app/app.vue), [nuxt.config.ts](/mnt/f/newworkspace/flareDocs/nuxt.config.ts)); homepage now exposes an "Install App" entry with iOS fallback guidance in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue) and [useAppLocale.ts](/mnt/f/newworkspace/flareDocs/app/composables/useAppLocale.ts).
- Fixed captcha false negatives in [captcha.ts](/mnt/f/newworkspace/flareDocs/app/server/utils/captcha.ts): captcha input normalization now applies Unicode `NFKC` conversion before whitespace cleanup, so visually identical full-width digits from CJK input methods no longer fail validation.
- Fixed captcha refresh staleness in [login.vue](/mnt/f/newworkspace/flareDocs/app/pages/login.vue): captcha reload requests now include a cache-busting nonce and apply last-request-wins guarding to avoid stale or out-of-order responses when users click refresh repeatedly.
- Fixed cross-device stale workspace/session behavior introduced by PWA caching: updated [sw.js](/mnt/f/newworkspace/flareDocs/public/sw.js) to never cache `/api/*` responses and only cache static assets, preventing old auth/workspace API data from persisting across refreshes or devices.
- Extended workspace list payload in [spaces/index.get.ts](/mnt/f/newworkspace/flareDocs/app/server/api/spaces/index.get.ts) with per-space `documentCount` and surfaced it under each homepage workspace name in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue).
- Fixed editor cursor jump during mobile/desktop typing in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue): background tree-triggered document reload now pauses during edit mode, and background refresh updates only metadata while editing to avoid resetting the draft model/cursor.
- Fixed voice panel persistence and mobile speech start path in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue): the floating voice panel no longer closes on autosave-driven document refresh or `Esc`, and mobile Chromium now skips the `getUserMedia` preflight so recognition starts directly from the user tap, which is more reliable on real devices; follow-up remains to re-check Android Chrome/Edge on device after deployment.
- Fixed local dev CSP interference in [00-security-headers.ts](/mnt/f/newworkspace/flareDocs/app/server/middleware/00-security-headers.ts): development no longer emits the custom CSP header at all, so Nuxt/Vite HMR plus dynamic imports are not blocked by the app's own security headers, while production keeps the stricter CSP.
- Reworked sharing navigation into the workspace shell in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue), [owned.get.ts](/mnt/f/newworkspace/flareDocs/app/server/api/shares/owned.get.ts), and [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): the left sidebar now exposes `文档 / 分享给我 / 我分享的`, both share lists reuse the main two-pane layout, received shares open as read-only detail, owned shares show recipient rows with revoke actions, and homepage share entry now deep-links into the workspace share view instead of a separate page; follow-up remains to decide whether legacy `/shared` pages should later redirect into this new shell.
- Refined the new share shell in [spaces/[spaceId].vue](/mnt/f/newworkspace/flareDocs/app/pages/spaces/[spaceId].vue) and [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): removed the redundant source card above shared-document content, changed `我分享的` to a single top-right action entry for recipient management, switched the left three-way view toggle to icon buttons, kept the mobile tree drawer open while only switching categories, and removed homepage share-preview data to keep the landing page simpler.
- Hardened refresh session persistence in [auth.ts](/mnt/f/newworkspace/flareDocs/app/server/utils/auth.ts): auth cookie now uses `SameSite=Lax` with explicit `expires` + `maxAge`, reducing refresh/login-loss cases across mobile and desktop browsers.
- Reworked homepage space-creation UX in [index.vue](/mnt/f/newworkspace/flareDocs/app/pages/index.vue): replaced inline create form with icon-triggered modal flow (matching management-modal interaction style) and changed default visibility to `private`.
- Unified `我分享的` workspace document shell with the normal docs view in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue): owned shares now reuse the same header/editor/reader/action menu flow as regular docs, with share management staying inside the share dialog instead of a separate top-level operation area.
- Aligned the owned-share dialog with the normal share dialog in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue): `我分享的` can now also add new share recipients directly from the same dialog, while the recipient list and revoke actions remain below.
- Reduced workspace entry latency in [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue): `分享给我` and `我分享的` lists are no longer fetched during the default docs first paint, and now load lazily on first entry into their respective views.
- Disabled service worker registration in [app.vue](/mnt/e/workspace/flareDocs/app/app.vue) and moved homepage/workspace primary data loads to client-side defaults in [index.vue](/mnt/e/workspace/flareDocs/app/pages/index.vue) and [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue), so route navigation is less likely to stall behind SSR fetches or stale SW-mediated asset fetches on remote servers.
- Added one-step homepage entry points for `分享给我 / 我分享的` in [index.vue](/mnt/e/workspace/flareDocs/app/pages/index.vue), and added a global top route progress bar for `/spaces/*` transitions in [app.vue](/mnt/e/workspace/flareDocs/app/app.vue) plus [main.css](/mnt/e/workspace/flareDocs/app/assets/css/main.css) so slow workspace entry has visible feedback.
- Switched homepage/workspace first-paint data loading from blocking `await useAsyncData(...)` to `useLazyAsyncData(...)` in [index.vue](/mnt/e/workspace/flareDocs/app/pages/index.vue) and [spaces/[spaceId].vue](/mnt/e/workspace/flareDocs/app/pages/spaces/[spaceId].vue), so route entry is no longer held until those requests resolve.
