...
- 文档树、文档 CRUD、乐观锁版本控制
- 个人工作区文档支持定向只读分享给指定用户；被分享人可在首页“分享给我”或独立只读页直接查看
...
- 登录链路已做首屏与重试优化：登录页不再阻塞等待 `/api/auth/me` 与验证码接口，验证码只在校验成功后作废，并增加前端重复提交保护，减少新设备首次登录慢和多次尝试的问题
- 工作区首开已减少非必要首屏开销：空间切换列表改为展开菜单时再请求，目录树首次进入默认不再全展开，同层文档改为按创建时间倒序展示，优先显示最近新建内容
- 工作区目录树已进一步改成按目录懒加载：首次仅请求根节点，点击文件夹时才加载该层子项；只有手动点击“全部展开”或打开“移动到...”时才全量抓取目录并显示进度
- 会话 Cookie 已改为更稳的代理 HTTPS 判定，并恢复 `SameSite=Strict`；同时移除了登录成功与 `/api/auth/me` 上的冗余个人工作区/默认管理员热路径检查，降低刷新后掉登录和登录后首屏变慢的风险
- 分享模型已与团队工作区职责分离：个人工作区适合草稿与定向只读分享；团队工作区只用于长期多人协作和团队沉淀，避免为一对一传阅制造过多团队空间
- 分享读取接口已兼容当前项目使用的 Drizzle 版本：不再依赖 `alias` 导出，避免首页或分享页请求时报 500
- 本地若未执行 `0004_document_shares.sql`，分享接口会返回明确的 migration 提示而不是直接抛 D1 表不存在堆栈；分享入口保留在右侧“更多工具”动作区，避免挤占文档头部空间
- 兼容早期个人工作区数据：即使旧空间名称仍是“某某 的个人/个人工作区”但 slug 不是新规则，也会继续识别为个人工作区并显示分享入口；同时将 ByteMD 改为静态加载，规避开发环境下动态导入失败导致的编辑区空白
...
- 工作区左栏继续重排：顶部改成“主页 + 当前分类”信息，空间切换下拉下沉到“文档/分享”内容区标题下方，手机端和 PC 端的关闭/导航关系更清晰
- 工作区左栏再次收敛：主页入口改成更明确的“返回主页”按钮，文档树操作折叠进单个菜单，给工作区下拉释放完整宽度
- 本地开发环境会主动卸载 flareDocs 的 service worker 并清理对应缓存，避免 `/_nuxt` 资源被旧缓存污染后出现“页面能开但按钮全失效”的假死问题
- 工作区左栏控制条进一步对齐：工作区下拉与“更多”按钮改为同一水平控制行，顶部按钮组和下拉区域的节奏更统一
- “更多”菜单按高频操作重排：`新增文档 / 新增目录` 提前到顶部，降低新建路径的操作成本
- 左栏主页入口继续收敛为纯图标按钮，保留语义提示但减少横向占用，把空间优先让给分类切换与工作区控制
- 团队工作区语义已回调到产品原意：所有已登录用户默认可见并可直接协作编辑文档；只有空间级管理动作（如成员维护、审计、删空间）仍通过管理员权限控制
- 文档动作菜单已统一补上“复制内容”；`分享给我` 视图补齐导出/复制/文档信息/全屏，`我分享的` 额外保留分享列表并支持一键回到原文档工作区继续完整编辑
- 文档信息弹框已补充“工作区”字段，并为 `分享给我 / 我分享的` 共用同一套共享文档信息展示，避免共享视图里点击信息无响应
...
- 语音悬浮面板不会再因自动保存或文档对象刷新被动关闭；关闭动作仅保留右上角显式关闭按钮。移动端 Chromium（Chrome/Edge 真机）语音启动改为直接从点击手势触发，避免先做麦克风预检后再启动识别时更容易出现的异常
- 若启用语音追加，服务端 `Permissions-Policy` 需允许同源麦克风访问（`microphone=(self)`），否则即使浏览器站点权限显示“允许”也会被策略头拦截
- 本地 `pnpm dev` 开发环境下不再下发自定义 CSP，避免 Nuxt/Vite 热更新与动态模块加载被应用自身响应头干扰；生产环境仍保持严格策略
- 工作区左侧已收敛为三种主视图：`文档`、`分享给我`、`我分享的`。分享列表和只读详情现在直接复用工作区双栏骨架；移动端也走同一套抽屉与右侧阅读布局，不再强依赖独立 `/shared` 页面跳转
- 分享详情顶部冗余来源卡片已移除；`我分享的` 改为右上角单一操作入口管理接收人；左栏三切换已收敛为图标按钮，且移动端切换分类时不再自动关闭目录抽屉；首页也移除了分享预览区，避免首屏信息过杂
...
- 编辑模式已避免后台刷新重置编辑器模型，减少手机端连续回车/输入时光标跳回顶部的问题
- 登录会话 Cookie 已调整为更稳妥的持久策略（`SameSite=Lax` + `maxAge/expires`），降低刷新后被要求重新登录的概率
- 首页创建空间已改为图标触发弹窗流程（不再内联展开创建表单），并将默认空间类型调整为“私有”
- 首页入口布局已精简：创建空间图标顺序调整到“人员管理”之后，右侧双描述区改为空间概览，左侧统计移除“空间总数”，并去掉“打开个人工作区”重复按钮
- 首页右侧会话面板已进一步改为“快速入门 4 步”：移除“打开工作区”入口，改成更清晰的上手步骤说明（中英文），便于新用户快速理解创建空间、建文档、编辑与协作流程
- 首页登录态右侧说明卡已改为“空间类型怎么选”：不再展示步骤列表，改为私有/团队/公开三种范围与适用场景说明，并给出“默认先私有再发布”的选择建议
- 首页登录态右侧说明卡已按交互反馈回调为“快速入门说明”样式，并移除该卡片中的空间数量标签，仅保留简洁步骤说明
- 深色皮肤下首页右侧“快速入门”卡片已补齐专属配色：步骤条目改为深色面板风格并增强文字对比，避免浅底突兀导致的观感不协调
- 移动端首页右上角操作区已优化：小屏隐藏 GitHub 图标并调整按钮顺序，减少拥挤导致的显示错乱；同时修复移动端“切换工作区后目录抽屉立即关闭”问题，现改为切换后保持抽屉打开，直到用户继续选择具体目录/文档
- 进一步优化移动端首页首屏：将“创建空间”从右上角图标区下移到主操作按钮区，并在小屏隐藏“安装到桌面”图标，降低首行拥挤和遮挡风险；首页标题与摘要文案也已改为更短、更直观的使用导向表达
...
- “我分享的”文档页继续向普通文档页收口：复用同一套标题栏、编辑器、阅读区和操作菜单，仅保留分享弹层内容与数据来源差异，减少双套页面维护。
- “我分享的”分享弹层也已补齐新增分享输入与提交，和普通文档保持同一套交互，只在目标文档来源上区分。
- 工作区首屏进一步瘦身：默认进入文档视图时不再同步请求“分享给我 / 我分享的”两组数据，改为切到对应分类时再懒加载，减少远端服务器下进入工作区的等待。
- 已停止注册前端 service worker，并将首页/工作区关键首屏数据改为客户端拉取+默认骨架态，减少远端服务器环境下登录后跳转和进入工作区时被 SSR 阻塞过久的问题。
- 首页主操作区已补充一步直达“分享给我 / 我分享的”入口；同时在进入 `/spaces/*` 工作区路由时显示全局顶部进度条，避免慢网环境下缺少反馈。
- 首页和工作区关键数据进一步改为 `useLazyAsyncData` 惰性拉取，路由会先进入页面，再异步补空间/目录/会话数据，避免等待全部请求完成才完成导航。
