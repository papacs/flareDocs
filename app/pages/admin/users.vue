...
const { data: meResponse } = await useAsyncData('admin-users-auth-me', () =>
...
const users = computed(() =>
  usersResponse.value && usersResponse.value.ok
...
const usersError = computed(() => {
  ...
  const fetchError = usersRequestError.value as FetchError<
    ApiResponse<never>
  > | null
  const apiMessage =
    fetchError?.data && !fetchError.data.ok
...
const canShowAdminPage = computed(() =>
  Boolean(currentUser.value?.isSystemAdmin)
)
...
async function createUser() {
  ...
  try {
    const response = await $fetch<ApiResponse<{ user: AdminUser }>>(
      '/api/admin/users',
      {
        method: 'POST',
        body: {
    ...
    createSuccess.value = t('adminUsers.createSuccess', {
    ...
    if (error instanceof FetchError) {
      ...
      createError.value =
        error instanceof Error ? error.message : t('adminUsers.createFailed')
    }
  } finally {
...
async function performUserAction(
  ...
  try {
    ...
    if (error instanceof FetchError) {
      ...
      actionError.value =
        error instanceof Error ? error.message : t('adminUsers.actionFailed')
    }
  } finally {
...
async function toggleUserAdmin(userItem: AdminUser) {
  await performUserAction(
    ...
    userItem.isSystemAdmin
      ? 'adminUsers.revokeAdminSuccess'
      : 'adminUsers.grantAdminSuccess'
  )
}
...
async function resetUserPassword(userItem: AdminUser) {
  ...
  const rawValue = window.prompt(
    t('adminUsers.resetPrompt', { username: userItem.username })
  )
  ...
  await performUserAction(
    ...
    'adminUsers.resetPasswordSuccess'
  )
}
...
async function deleteUser(userItem: AdminUser) {
  ...
  if (
    !window.confirm(
      t('adminUsers.deleteConfirm', { username: userItem.username })
    )
  ) {
    return
  }
  ...
  try {
    const response = await $fetch<
      ApiResponse<{ deleted: boolean; userId: number }>
    >(`/api/admin/users/${userItem.id}`, {
      method: 'DELETE',
      body: {}
    })
    ...
    actionMessage.value = t('adminUsers.deleteSuccess', {
...
<template>
  <main
    ...
    class="fd-admin-shell mx-auto flex h-dvh w-full max-w-6xl flex-col gap-2 overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(248,242,231,0.92),transparent_40%),linear-gradient(180deg,#f6f1e8_0%,#f4efe6_100%)] px-3 py-2 sm:gap-5 sm:px-6 sm:py-4 lg:px-8"
  >
    <header
      class="fd-admin-card relative rounded-2xl border border-[rgba(31,41,55,0.1)] bg-white/80 p-3.5 shadow-[0_16px_36px_rgba(120,98,69,0.1)] sm:rounded-3xl sm:p-5"
    >
      <div class="pr-10 sm:pr-12">
        <div>
          ...
          <h1
            class="mt-1.5 text-[2rem] font-semibold leading-tight text-slate-800 sm:mt-2 sm:text-3xl"
          >
            {{ t('adminUsers.title') }}
          </h1>
          <p class="mt-1.5 text-sm text-slate-500 sm:mt-2">
            {{ t('adminUsers.summary') }}
          </p>
        </div>
        <UButton
          class="absolute right-2.5 top-2.5 sm:right-4 sm:top-4"
          color="neutral"
          variant="ghost"
          to="/"
          :title="t('adminUsers.backHome')"
        >
    ...
    <section
      class="grid gap-3 sm:gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]"
    >
      <article
        class="fd-admin-card rounded-2xl border border-[rgba(31,41,55,0.1)] bg-white/80 p-4 shadow-[0_14px_32px_rgba(120,98,69,0.1)] sm:rounded-3xl sm:p-5"
      >
        ...
        <form class="mt-4 space-y-3" @submit.prevent="createUser">
          ...
          <label class="flex items-center gap-2 text-sm text-slate-600">
            <input
              v-model="createForm.isSystemAdmin"
              type="checkbox"
      ...
      >
        ...
        <p
          v-if="usersError"
          class="mt-3 rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-600"
        >
          {{ usersError }}
        </p>
        <p
        ...
        <p
          v-if="actionMessage"
          class="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-sm text-emerald-700"
        >
          {{ actionMessage }}
        </p>
        ...
        <ul v-else-if="users.length > 0" class="mt-4 space-y-2">
          <li
            ...
            :key="userItem.id"
            class="fd-admin-user-row flex flex-col items-stretch gap-2 rounded-2xl border border-[rgba(31,41,55,0.08)] px-3 py-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3"
            :class="
              userItem.isActive
                ? 'bg-[rgba(250,247,241,0.9)]'
                : 'bg-[rgba(241,238,232,0.86)] opacity-75'
            "
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                <p class="truncate text-sm font-semibold text-slate-800">
                  {{ userItem.username }}
                  <span v-if="!userItem.isActive" class="ml-1 text-rose-600"
                    >({{ t('adminUsers.disabled') }})</span
                  >
                </p>
                <p class="text-xs text-slate-500">
                  {{ formatTimestamp(userItem.createdAt) }}
                </p>
              </div>
            </div>
            <div
              class="grid w-full grid-cols-2 gap-1.5 sm:flex sm:w-auto sm:flex-wrap sm:items-center sm:justify-end sm:gap-2"
            >
              <span
                v-if="currentUser && currentUser.id === userItem.id"
                class="col-span-2 text-[11px] text-slate-400 sm:col-auto"
              >
                {{ t('adminUsers.currentUser') }}
              </span>
              <span
                class="col-span-2 text-xs font-medium text-slate-500 sm:col-auto"
              >
                {{
                  userItem.isSystemAdmin
                    ? t('index.systemAdmin')
                    : t('adminUsers.normalUser')
                }}
              </span>
              <UButton
                size="xs"
